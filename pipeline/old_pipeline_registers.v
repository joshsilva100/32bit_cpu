...
// reset   (input) - set to 1 to set all registers to zero, set to 0 for normal execution. Decide if we keep this or not 
// flush  (input) - When high, hazard unit wants to make a "bubble". Set all WB, M, EX control signals to low 
...
// branch_alu_a (input) - Output from plus4 alu
// 
// OUTPUTS: 
//
...
// out_branch_alu_a (output) - Output from plus4 alu. Will be used later if branch taken

module if_id_reg(clk, reset, flush,
                if_id_write, inst, link_data, branch_alu_a,
                out_inst, out_link_data, out_branch_alu_a
                ); 

    input clk, reset, flush, if_id_write; 
    input [31:0] inst, link_data, branch_alu_a;

    output reg [31:0] out_inst, out_link_data, out_branch_alu_a; 
    ...
    always @(posedge clk or posedge reset) begin
        ...
        if (reset) begin
            ...
            out_branch_alu_a <= 32'h00000000;
        end 

        // Jump or Branch taken. Hazard Unit does not want this instruction!
        else if (flush) begin 
            out_inst <= 32'h00000000; 
            out_link_data <= 32'h00000000;  
            out_branch_alu_a <= 32'h00000000;
        ...
        // Normal Operation
            ...
            out_branch_alu_a <= branch_alu_a;
        end

        // Data Hazard (Load use)! Stall until hazard unit says otherwise
        else begin
            out_inst <= out_inst; 
            out_link_data <= out_link_data; 
            out_branch_alu_a <= out_branch_alu_a;
        end
	end
...
//
// clk     (input) - the clock signal
// reset   (input) - When high, sets all registers to zero. Just making it a reset, decide if we keep this or not  
// flush  (input) - When high, hazard unit wants to make a "bubble". Set all WB, M, EX control signals to low
//
// WB Control Group
...
// shift_v (input) - SHIFT_V control signal from MIPS decoder. Determines inB input for Shift Module
// shift_op (input) - SHIFT_OP control signal from the MIPS decoder. Determines shift operation for Shift Module 
// branch (input) - BRANCH control signal from the MIPS decoder. Lets branch module know a branch instruction was feed, which then decides whether or not to take the branch
// 
...
// branch_alu_a (input) - Plus4 ALU output
// 
// OUTPUTS: 
//
// WB Control Group
...
// out_byte_load (output) - Decides if loading a byte or whole word. Sending/Holding for M stage
// 
// EX Control Group
...
// out_load_ra (output) - Acts as part of RegDst. Determines if data is from $RA or from Rd/Rt. Send to EX stage
// out_alu_op (output) - Determines operation of primary ALU. Send to EX stage 
// out_slt (output) - SLT control signal from MIPS decoder. Will decide between ALU_out or other control signals in EX stage. Send to EX stage 
// out_shift (output) - SHIFT control signal from MIPS decoder. Will decide between ALU_out or other control signals in EX stage. Send to EX stage 
// out_lui (output) - LUI control signal from MIPS decoder. Will decide between ALU_out or other control signals in EX stage. Send to EX stage 
// out_link (output) - LINK control signal from MIPS decoder. Will decide between ALU_out or other control signals in EX stage. Send to EX stage 
// out_shift_v (output) - SHIFT_V control signal from MIPS decoder. Determines inB input for Shift Module. Send to EX stage
// out_shift_op (output) - SHIFT_OP control signal from the MIPS decoder. Determines shift operation for Shift Module. Send to EX stage 
// out_branch (output) - BRANCH control signal from the MIPS decoder. Lets branch module know a branch instruction was feed, which then decides whether or not to take the branch. Send to EX stage
// 
...
// out_lui_data (output) - The upper immediate data. Sending to EX stage
// out_link_data (output) - Link ALU output. Take from PC. Sending to EX stage 
// out_branch_alu_a (input) - Plus4 ALU output. Used later if branch taken for branch alu

module id_ex_reg(clk, reset, flush
                write, mem_read, //WB
                word_we, byte_we, byte_load, //M
                i_type, load_ra, alu_op, slt, shift, lui, link, shift_v, shift_op, branch, //EX
                rsData, rtData, imm32, rt, rd, rs, mux_shift_v_A, lui_data, link_data, branch_alu_a, //Other 
                out_write, out_mem_read, //WB
                out_word_we, out_byte_we, out_byte_load, //M
                out_i_type, out_load_ra, out_alu_op, out_slt, out_shift, out_lui, out_link, out_shift_v, out_shift_op, out_branch, //EX
                out_rsData, out_rtData, out_imm32, out_rt, out_rd, out_rs, out_mux_shift_v_A, out_lui_data, out_link_data, out_branch_alu_a//Other
            ); 

    input clk, reset, flush, write, mem_read, word_we, byte_we, byte_load, i_type, load_ra, slt, shift, lui, link, shift_v, branch; 
    input [2:0] alu_op; 
    input [1:0] shift_op;
    input [31:0] rsData, rtData, imm32, mux_shift_v_A, lui_data, link_data, branch_alu_a; 
    input [4:0] rt, rd, rs;

    output reg out_write, out_mem_read, out_word_we, out_byte_we, out_byte_load, out_i_type, out_load_ra, out_slt, out_shift, out_lui, out_link, out_shift_v, out_branch; 
    output reg [2:0] out_alu_op; 
    output reg [1:0] out_shift_op;
    output reg [31:0] out_rsData, out_rtData, out_imm32, out_mux_shift_v_A, out_lui_data, out_link_data, out_branch_alu_a; 
    ...
    always @(posedge clk or posedge reset) begin
        ...
        // HAZARD! Stall by setting all WB, M, EX control signals to low
        else if (flush) begin 
            // WB Control Group
            out_write <= 0;
            out_mem_read <= 0; 
...
module ex_mem_reg(clk, reset, 
    ...
    input clk, reset, write, mem_read, word_we, byte_we, byte_load; 
    input [4:0] rdNum; 
